...
# ─── Database ──────────────────────────────────────────────────────────────
# SQLite by default for local dev; set DB_ENGINE=postgres in .env to switch
# to Postgres in production. No code change needed — only .env values.
if os.getenv("DB_ENGINE", "").strip().lower() == "postgres":
    DATABASES = {
        "default": {
